...
print(
    'docker-compose run --rm -v {:}:/panels -e "ID={:}" -e "INSTID={:}" -e "PANEL={:}" scout-addpanel'.format(
        os.path.abspath(args.folder), args.panel, args.instid, args.panel
    )
)
os.system(
    'docker-compose run --rm -e "ID={:}" -e "INSTID={:}" -e "PANEL={:}" scout-addpanel'.format(
        args.id, args.instid, args.panel
    )
)
