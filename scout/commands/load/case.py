...
def case(
    ...
    try:
        ...
        LOG.error("SyntaxError {} missing when loading '{}' {}".format(err, config.name, traceback.format_exc()))
        raise click.Abort()
    except KeyError as err:
        LOG.error("KeyError {} when loading '{}' {}".format(err, config.name, traceback.format_exc()))
        raise click.Abort()
