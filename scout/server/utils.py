...
def count_cursor(cursor):
    ...
    new_cursor2 = cursor.clone()  # Clone cursor, otherwise original will move
    new_cursor3 = cursor.clone()  # Clone cursor, otherwise original will move
    ...
    # LOG.debug("size(): {}".format(new_cursor.collection.estimated_document_count(new_cursor.get('_Cursor__spec'))))
    # LOG.debug("stat(): {}".format(new_cursor.stats))
    # LOG.debug("size(): {}".format(new_cursor.collection.estimated_document_count( {'case_id': 'cancer_test1', 'category': 'cancer_sv', 'variant_type': 'clinical'} )))
