...
def add_tx_links(tx_obj, build=37, hgnc_symbol=None):
    ...
    tx_obj["vutr_link"] = vutr(tx_obj)

    return tx_obj


def vutr(tx: dict) -> str | None:
    """Return link to VuTR viewer, if transcript exists and is canonical."""

    if not tx:
        return None

    ensembl_tx_id = tx.get("transcript_id")

    if not all([ensembl_tx_id, tx.get("is_canonical")]):
        return None

    return f"https://vutr.rarediseasegenomics.org/viewer/{ensembl_tx_id}"
