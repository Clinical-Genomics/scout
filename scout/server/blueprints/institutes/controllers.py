...
def update_institute_settings(store: MongoAdapter, institute_obj: Dict, form: MultiDict) -> Dict:
    ...
    updated_institute = store.update_institute(
        ...
        add_groups=False,
        sharing_institutes=get_admin_setting_list(field_name="institutes", form=form),
        cohorts=cohorts,
        loqusdb_ids=get_admin_setting_list(field_name="loqusdb_id", form=form),
...
def case_has_rna(case: dict) -> bool:
    ...
    if any(
        exists(omics.get(key))
        for key in ("fraser", "outrider", "fraser_research", "outrider_research")
    ):
        return True
    ...
    for ind in case.get("individuals", []):
        if any(
            exists(ind.get(key))
            for key in ("rna_alignment_path", "splice_junctions_bed", "rna_coverage_bigwig")
        ):
            return True
