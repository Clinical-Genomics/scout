...
def cancer_variants(store, institute_id, case_name, variants_query, variant_count, form, page=1):
    ...
    variant_count_session(store, institute_id, case_obj, "clinical", "cancer")
...
def variant_count_session(store, institute_id, case, var_type, var_category):
    ...
    """
    # Run the query just once for a cases
    if (
        session.get("institute") != institute_id
        or session.get("case") != case["_id"]
        or session["case_variants"] is None
    ):
        session["case"] = case["_id"]
        session["institute"] = institute_id
        case_variants = case.get("variants_stats") or store.case_variants_count(
            case["_id"], institute_id, True
        )
