...
def variant_verification(
    ...
    breakpoint_1 = ":".join([chromosome, str(variant_obj["position"])]) if category in ["sv","cancer_sv"] else "-"
    breakpoint_2 = ":".join([end_chrom, str(variant_obj.get("end"))]) if category in ["sv","cancer_sv"] else "-"
