...
def genomic_features(store, case_obj, sample_name, genes_only):
    ...
    for var in individual_pinned:
        ...
        if var["category"] == "sv" and len(hgnc_genes) > 3:
            continue
        ...
        for hgnc_id in hgnc_genes:
            ...
            if (
                genes_only or var["category"] == "sv"
            ) and not hgnc_id in gene_set:  # if only gene names should be shared
