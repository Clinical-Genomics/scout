...
def test_index(mock_app):
    ...
    result =  runner.invoke(app_cli, ['index'])
    # It should print confirm message
    assert 'This will delete and rebuild all indexes(if not --update). Are you sure? [y/N]' in result.output

    # Provide confirm command to CLI (say yes)
    result =  runner.invoke(app_cli, ['index'], input='y')
    assert result.exit_code == 0
    assert 'INFO creating indexes' in result.output

    # Provide confirm command and update option
    result =  runner.invoke(app_cli, ['index', '--update'], input='y')
    assert result.exit_code == 0
