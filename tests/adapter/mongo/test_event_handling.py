...
def test_add_phenotype_group(adapter, institute_obj, case_obj, user_obj):
    ...
    assert sum(1 for _ in adapter.hpo_term_collection.find()) > 0
...
def test_filter_stash(adapter, institute_obj, case_obj, user_obj, filter_obj):
    ...
    assert sum(1 for _ in filters) == 0
    ...
    assert sum(1 for _ in filters) == 1
...
def test_filter_audit_unaudit(adapter, institute_obj, case_obj, user_obj, filter_obj):
    ...
    # THEN an event can be found in the event collection
    n_events_before = sum(1 for _ in adapter.event_collection.find())
