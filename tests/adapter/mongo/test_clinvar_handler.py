...
def get_new_submission(adapter, user_obj, institute_obj):
    ...
    new_submission_id = adapter.create_submission(
        user_id=user_id, institute_id=institute_id
    )
...
def test_clinvar_submission_status(adapter, user_obj, institute_obj):
    ...
    updated_submission = adapter.update_clinvar_submission_status(
        user_id, submission_obj["_id"], "closed"
    )
...
def test_add_remove_subm_objects(adapter, user_obj, institute_obj, case_obj):
    ...
    # Get the ID of an existing submission
    submission_obj = adapter.get_open_clinvar_submission(
        user_id=user_obj["_id"], institute_id=institute_obj["_id"]
    )
