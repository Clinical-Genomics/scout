...
def test_create_event(pymongo_adapter, institute_obj, case_obj, user_obj):
    institute_obj['_id'] = institute_obj['internal_id']
    user_obj['_id'] = user_obj['email']
    
    ## GIVEN a database without any events
    
    assert pymongo_adapter.event_collection.find().count() == 0
    
    ## WHEN inserting a event
    verb = "status"
    pymongo_adapter.create_event(
    ...
    pymongo_adapter.event_collection.find().count() == 1
    res = pymongo_adapter.event_collection.find_one()
