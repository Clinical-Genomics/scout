...
def test_check_reset_variant_ccv_classification(app, case_obj, variant_obj):
    """Test the check_reset_variant_ccv_classification controller
    Assume we are in the situation when it will trigger to clear classifications,
    and make sure it does clear the classification."""
    ...
    with app.test_client() as client:
        ...
        # GIVEN a variant classified according to ClinGen-CGC-VIGG
        assert store.variant_collection.find_one_and_update(
            {"_id": variant_obj["_id"]},
            {"$set": {"ccv_classification": 1}},
        )

        # WHEN retrieving the variant again,
        current_variant = store.variant(variant_obj["_id"])
        # THEN the classification was acutally set
        assert current_variant.get("ccv_classification") is not None

        # GIVEN that there are no separate evaluations stored for the variant
        assert len(list(store.get_ccv_evaluations_case_specific(variant_obj["document_id"]))) == 0
        ...
        # WHEN checking if variant evaluations have been removed,
        # THEN it returns true
        assert check_reset_variant_ccv_classification(store, deleted_evaluation_obj, "link")

        # WHEN retrieving the variant again,
        current_variant = store.variant(variant_obj["_id"])
        # THEN the classification has now been cleared
        assert current_variant.get("ccv_classification") is None
