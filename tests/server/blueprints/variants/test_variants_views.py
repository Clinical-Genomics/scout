...
def test_bulk_reset_dismiss_variants(app, institute_obj, case_obj, mocker, mock_redirect):
    ...
    with app.test_client() as client:
        ...
        assert sum(1 for _ in store.event_collection.find({"category": "variant"})) == 2

        # And one case-specific event should also be created
        assert (
            sum(
                1
                for _ in store.event_collection.find(
...
def test_mei_variants_research(app, institute_obj, case_obj):
    with app.test_client() as client:
        # GIVEN that the user could be logged in
        resp = client.get(url_for("auto_login"))
        assert resp.status_code == 200
...
def test_mei_variants_post_data(app, institute_obj, case_obj):
    # GIVEN an initialized app
    # GIVEN a valid user and institute
