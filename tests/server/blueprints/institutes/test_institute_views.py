...
def test_remove_phenomodel(app, user_obj, institute_obj, mocker, mock_redirect):
    ...
    mocker.patch(
        "scout.server.blueprints.institutes.views.redirect", return_value=mock_redirect
    )
...
def test_phenomodel_POST_add_delete_subpanel(app, user_obj, institute_obj):
    ...
    with app.test_client() as client:
        ...
        assert (
            updated_model["subpanels"][subpanel_id]["title"]
            == "Phenotype subpanel title"
        )
        assert (
            updated_model["subpanels"][subpanel_id]["subtitle"]
            == "Phenotype subpanel subtitle"
        )
...
def test_phenomodel_POST_add_omim_checkbox_to_subpanel(
    app, user_obj, institute_obj, omim_checkbox
):
...
def test_phenomodel_POST_add_hpo_checkbox_to_subpanel(
    app, user_obj, institute_obj, hpo_checkboxes
):
...
def test_gene_variants_no_valid_gene(
...
def test_filters(app, institute_obj, user_obj, case_obj, filter_obj):
    ...
    with app.test_client() as client:
        ...
        # WHEN accessing the cases page
        resp = client.get(
            url_for("overview.filters", institute_id=institute_obj["internal_id"])
        )
...
def test_rename_clinvar_samples(app, institute_obj, clinvar_variant, clinvar_casedata):
    ...
    with app.test_client() as client:
        ...
        referer = url_for(
            "overview.clinvar_submissions", institute_id=institute_obj["internal_id"]
        )
